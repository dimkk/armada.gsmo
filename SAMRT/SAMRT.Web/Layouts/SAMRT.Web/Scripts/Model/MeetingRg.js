...
(function () {
    ...
    function openParticipantSelectDialog(title, container, isMulti) {
        var options = {
            title: title,
            url: isMulti ? _spPageContextInfo.webAbsoluteUrl + "/_layouts/15/SAMRT.Web/Pages/ParticipantsSelection.aspx?Multi=1" : _spPageContextInfo.webAbsoluteUrl + "/_layouts/15/SAMRT.Web/Pages/ParticipantsSelection.aspx",
            width: 800,
            height: 400,
    ...
    function openQuestionAttachmentsDialog(title, id) {
        var options = {
            title: title,
            url: _spPageContextInfo.webAbsoluteUrl + (String).format("/_layouts/15/SAMRT.Web/Pages/IssueAttachmentMVK.aspx?ID={0}", id),
            width: 800,
            height: 400,
            dialogReturnValueCallback: function (dialogResult, returnValue) {
                // ToDo: 
    ...
    function openQuestionObjectsDialog(title, data) {
        ...
        if (data.agendaQuestionObjects().length > 1)
            url = url + (String).format("/Lists/IssueObjectMVKList/AllItems.aspx?FilterField1=IssueObjectIssueIdMVK&FilterValue1={0}", data.Id());
        else
            url = url + (String).format("/Lists/Object/DispForm.aspx?ID={0}", data.agendaQuestionObjects()[0].IssueObjectIssueIdMVK().get_lookupId());
        var options = {
            title: title,
            url: url,
            width: 1024,
    ...
    function openOrderRgAddDialog(title) {
        var options = {
            title: title,
            url: _spPageContextInfo.webAbsoluteUrl + "/Lists/OrderMVKList/Unused.aspx",
            width: 1024,
            height: 768,
            dialogReturnValueCallback: function (dialogResult, returnValue) {
    ...
    function openIssueRgCreateDialog(title) {
        var options = {
            title: title,
            url: _spPageContextInfo.webAbsoluteUrl + "/Lists/IssueMVKList/NewForm2.aspx",
            width: 1024,
            height: 768,
            dialogReturnValueCallback: function (dialogResult, returnValue) {
				SP.UI.ModalDialog.RefreshPage(dialogResult);
    ...
    function Assignment(data) {
        ...
        selfAssign.selectExecutor = function () {
            var options = {
                ...
                url: _spPageContextInfo.webAbsoluteUrl + "/_layouts/15/SAMRT.Web/Pages/ExecutorSelection.aspx",
                width: 800,
                height: 400,
                dialogReturnValueCallback: function(dialogResult, returnValue) {
        ...
        selfAssign.selectSoexecutors = function () {
            var options = {
                ...
                url: _spPageContextInfo.webAbsoluteUrl + "/_layouts/15/SAMRT.Web/Pages/ParticipantsSelection.aspx?Multi=1",
                width: 800,
                height: 400,
                dialogReturnValueCallback: function (dialogResult, returnValue) {
    ...
    function agendaQuestion(data) {
        ...
        this.calcContent = ko.computed(function () {
            ...
            //str += "Тип проекта: ???" + ";";
            str += this.IssueAddressMVK() ? "Адрес объекта: " + this.IssueAddressMVK() + ";" : "";
            //str += this.IssueInvestorMVK() ? "Инвестор: " + this.IssueInvestorMVK() + ";" : "";

            return str || this.IssueDescriptionMVK();
        }, this);

        // вычислимое поле для списка докладчиков
        this.calcReporters = ko.computed(function () {
            var reporters = [];
            if (this.IssueReporterRg()) {
                reporters.push(this.IssueReporterRg().trim());
            } else {
                if (this.IssueReporterMVK && this.IssueReporterMVK()) {
            ...
            if (this.agendaQuestionSoreporters && this.agendaQuestionSoreporters() != null && this.agendaQuestionSoreporters().length > 0) {
                ...
                if (this.IssueCoReportesMVK() != null) {
                    $.each(this.IssueCoReportesMVK(), function () {
        ...
        this.httpLink = ko.observable(
            (String).format("{0}//{1}/sites/gca/Lists/IssueMVKList/DispForm2.aspx?ID={2}&ContentTypeId={3}",
		...
		this.gotoEditQuestion = function() {
			window.open(_spPageContextInfo.webAbsoluteUrl + '/Lists/IssueMVKList/DispForm2.aspx?ID=' + data.Id, '_blank');
		}
        ...
        //        url: _spPageContextInfo.webAbsoluteUrl + "/_layouts/15/SAMRT.Web/Pages/SearchQuestion.aspx",
        //        width: 800,
        //        height: 400,
        //        dialogReturnValueCallback: function (dialogResult, returnValue) {
    ...
    function meeting(data) {
        ...
        this.changeTitle = ko.computed(function () {
            var number = this.MeetingNumberMVK();
            var nowDate = new Date();
    ...
    function meetingViewModel() {
        ...
        self.loadData = function () {
            self.availableMeetingStatuses(getChoicesFromMetadata(modelMetaData.meeting.fields, "MeetingStatusMVK"));
            ...
            if ($.pageMode == "new" || $.listItemId == "undefined") {
                ...
                var aqCamlQuery = new CamlBuilder().Where().LookupField("IssueMeetingMVK").Id().EqualTo($.listItemId).OrderBy("IssueNumberMVK");
                ...
                // loading attachments
                var atCamlQuery = new CamlBuilder().Where().LookupField("MeetingAttachmentMeetingMVK").Id().EqualTo($.listItemId);
                ...
                $.appWebContext.executeQueryAsync(function () {
                    try
                    {
                        ...
                        while (enumerator.moveNext()) {
                            ...
                            if (newQuest.IssueReporterMVK && newQuest.IssueReporterMVK() != null)
                                allMeetingBaseParticipants.push(newQuest.IssueReporterMVK());
                            if (newQuest.IssueCoReportesMVK() != null)
                                $.each(newQuest.IssueCoReportesMVK(),
                        ...
                        while (enumerator.moveNext()) {
                            var docTypeLink = enumerator.get_current().get_item("MeetingAttachmentDocTypeMVK");
                            ...
                            attach = new meetingAttach({
                                AttachmentDescription: enumerator.get_current().get_item("MeetingAttachmentDescriptionMVK"),
                                MeetingLink: enumerator.get_current().get_item("MeetingAttachmentMeetingMVK"),
                                Id: enumerator.get_current().get_item("ID"),
                                FileUrl: fileInfo.FileUrl,
                                FileName: fileInfo.FileName,
                                FilePath: "",
                            ...
                            if (enumerator.get_current().get_item("MeetingAttachmentProtocolCopyMVK")) {
        ...
        self.removeAgendaQuestion = function (agendaQuestion) {
			var currentNumber = agendaQuestion.IssueNumberMVK();
			...
			for (var i = questions.indexOf(agendaQuestion) + 1; i < questions.length; i++) {
				...
				listItem.set_item('IssueNumberMVK', number);
				listItem.update();
				number++;
			}
			...
			$.appWebContext.executeQueryAsync(function () {
				...
				for (var i = questions.indexOf(agendaQuestion) + 1; i < questions.length; i++) {
					questions[i].IssueNumberMVK(number);
					number++;
				}
		...
		self.moveAgendaQuestion = function (current, isUp) {
			...
			var currentNumber = current.IssueNumberMVK();
			var prevNumber = prev.IssueNumberMVK();
			...
			$.appWebContext.executeQueryAsync(function () {
					...
					prevListItem.set_item('IssueNumberMVK', currentNumber);
					currentListItem.set_item('IssueNumberMVK', prevNumber);
					prevListItem.update();
					currentListItem.update();
			
					$.appWebContext.executeQueryAsync(function () {
						console.log("Move question " + current.Id() + " success");
						prev.IssueNumberMVK(currentNumber);
						current.IssueNumberMVK(prevNumber);
						self.agendaQuestions(self.agendaQuestions().sort(function(a, b) {
							return a.IssueNumberMVK() > b.IssueNumberMVK() ? 1 : -1;
						}))
					},
					function (sender, args) {
							alert('Request failed. ' + args.get_message() +
