...
(function () {
    ...
    function openQuestionAttachmentsDialog(title, id) {
        var options = {
            title: title,
            url: _spPageContextInfo.webAbsoluteUrl + (String).format("/_layouts/15/SAMRT.Web/Pages/AgendaQuestionAttach.aspx?ID={0}", id),
            width: 800,
            height: 400,
            dialogReturnValueCallback: function (dialogResult, returnValue) {
                // ToDo: 
    ...
    function openIssueAddDialog(title) {
        var options = {
            title: title,
            url: _spPageContextInfo.webAbsoluteUrl + "/Lists/IssueMVKList/UnusedAll.aspx",
            width: 1024,
            height: 768,
            dialogReturnValueCallback: function (dialogResult, returnValue) {
				if (dialogResult == SP.UI.DialogResult.OK) {
					var selectedIssuesId = JSON.parse(returnValue);
					if (selectedIssuesId.length <= 0)
						return;
					console.log(selectedIssuesId);
					var queryUrl = _spPageContextInfo.webServerRelativeUrl + _spPageContextInfo.serverRequestPath + "/AddIssuesRg";
			        var newData = { meetingAkId: $.listItemId };
			        newData.issueRgIdList = [];
				    selectedIssuesId.map(function(el) { newData.issueRgIdList.push(el.id); });
			        $.ajax({
                        url: encodeURI(queryUrl),
    ...
    function openIssueCreateDialog(title) {
        var options = {
            title: title,
            url: _spPageContextInfo.webAbsoluteUrl + "/Lists/AgendaQuestionList/NewForm.aspx",
            width: 1024,
            height: 768,
            dialogReturnValueCallback: function (dialogResult, returnValue) {
				SP.UI.ModalDialog.RefreshPage(dialogResult);
    ...
    function agendaQuestion(data) {
        ...
        this.calcReporters = ko.computed(function () {
            var reporters = [];
            if (this.IssueReporterAk()) {
                reporters.push(this.IssueReporterAk().trim());
            } else {
        ...
        this.httpLink = ko.observable(
            (String).format("{0}//{1}/sites/gca/Lists/AgendaQuestionList/DispForm.aspx?ID={2}&ContentTypeId={3}",
		...
		this.gotoEditQuestion = function() {
			window.open(_spPageContextInfo.webAbsoluteUrl + '/Lists/AgendaQuestionList/DispForm.aspx?ID=' + data.Id, '_blank');
		}
    ...
    function meetingViewModel() {
		...
		self.addIssue = function() {
			openIssueAddDialog("Добавить вопросы Рабочей группы");
		}
		
		self.createIssue = function() {
			openIssueCreateDialog("Создать вопрос Архитектурной комиссии");
		}

        // remove agenda question
        self.removeAgendaQuestion = function (agendaQuestion) {
			var currentNumber = agendaQuestion.AgendaQuestionNumber();
			...
			for (var i = questions.indexOf(agendaQuestion) + 1; i < questions.length; i++) {
				...
				listItem.set_item('AgendaQuestionNumber', number);
				listItem.update();
				number++;
			}
			...
			$.appWebContext.executeQueryAsync(function () {
				...
				for (var i = questions.indexOf(agendaQuestion) + 1; i < questions.length; i++) {
					questions[i].AgendaQuestionNumber(number);
					number++;
				}
		...
		self.moveAgendaQuestion = function (current, isUp) {
			...
			var currentNumber = current.AgendaQuestionNumber();
			var prevNumber = prev.AgendaQuestionNumber();
			...
			$.appWebContext.executeQueryAsync(function () {
					...
					prevListItem.set_item('AgendaQuestionNumber', currentNumber);
					currentListItem.set_item('AgendaQuestionNumber', prevNumber);
					prevListItem.update();
					currentListItem.update();
			
					$.appWebContext.executeQueryAsync(function () {
						console.log("Move question " + current.Id() + " success");
						prev.AgendaQuestionNumber(currentNumber);
						current.AgendaQuestionNumber(prevNumber);
						self.agendaQuestions(self.agendaQuestions().sort(function(a, b) {
							return a.AgendaQuestionNumber() > b.AgendaQuestionNumber() ? 1 : -1;
						}))
					},
					function (sender, args) {
							alert('Request failed. ' + args.get_message() +
