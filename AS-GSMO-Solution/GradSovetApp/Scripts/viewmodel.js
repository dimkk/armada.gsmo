...
(function () {
    ...
    function saveAgendaQuestionsMeasures(questions) {
        var list = $.appWebContext.get_web().get_lists().getByTitle("Вложения вопроса повестки");
        var newItems = [];
        for (var question in questions) {
            for (var m in questions[question].agendaQuestionMeasures()) {
                var current = questions[question].agendaQuestionMeasures()[m];
                // Build a request up to send with the CSOM.
                if (current._destroy) {
                    // Handle deleted objects
                    // Deleted items that are marked "new" have never been saved to SharePoint to start with
                    ...
                    listItem.set_item("Question", questions[question].Id());
                    listItem.set_item("Measure", current.MeasureId);
                    listItem.set_item("Value", current.Value);
                    listItem.update();
    ...
    function getMeasureTypes() {
        var result = [];
        var fileQuery = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getByTitle('Типы показателей')/items";
        $.ajax({
            url: fileQuery,
            headers: {
                Accept: "application/json;odata=verbose"
            ...
            success: function (data) {
                if (data) {
                    for (i = 0; i < data.d.results.length; i++) {
                        result.push(new MeasureType(data.d.results[i]));
                    }
                }
            }
        });
    ...
    function getMeasuringUnits() {
        var result = [];
        var fileQuery = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getByTitle('Единицы измерения показателей')/items";
        $.ajax({
            url: fileQuery,
            headers: {
                Accept: "application/json;odata=verbose"
            ...
            success: function (data) {
                if (data) {
                    for (i = 0; i < data.d.results.length; i++) {
                        result.push(new MeasuringUnit(data.d.results[i]));
                    }
                }
            }
        });
    ...
    function openParticipantSelectDialog(title, container, isMulti) {
        var options = {
            title: title,
            url: isMulti ? _spPageContextInfo.webAbsoluteUrl + "/Pages/ParticipantsSelection.aspx?Multi=1" : _spPageContextInfo.webAbsoluteUrl + "/Pages/ParticipantsSelection.aspx",
            width: 800,
            height: 400,
    ...
    function Measure(data) {
        ...
        this.MeasureTypeId = data.MeasureTypeId;
        this.UnitId = data.UnitId;
    }

    function MeasureType(data) {
        this.Id = data.Id;
        this.Name = data.Title;
    }

    function MeasuringUnit(data) {
        this.Id = data.Id;
        this.Name = data.Title;
    }
    ...
    function Assignment(data) {
        ...
        selfAssign.selectExecutor = function () {
            var options = {
                ...
                url: _spPageContextInfo.webAbsoluteUrl + "/Pages/ExecutorSelection.aspx",
                width: 800,
                height: 400,
                dialogReturnValueCallback: function(dialogResult, returnValue) {
        ...
        selfAssign.selectSoexecutors = function () {
            var options = {
                ...
                url: _spPageContextInfo.webAbsoluteUrl + "/Pages/ParticipantsSelection.aspx?Multi=1",
                width: 800,
                height: 400,
                dialogReturnValueCallback: function (dialogResult, returnValue) {
    ...
    // вопрос повестки
    function agendaQuestion(viewModel, data) {
        ...
        for (var prop in data) {
            if (prop == 'New') {
                ...
                // property fo editing
                this["edit" + prop] = ko.observable(data[prop]);
                if (prop == "AgendaLinkedQuestionLink" || prop == "QuestionCategoryLink") {
                    this[prop + "Value"] = ko.observable(data[prop] ? data[prop].get_lookupValue() : null);
                    this[prop + "Id"] = ko.observable(data[prop] ? data[prop].get_lookupId() : null);
                    this["edit" + prop + "Value"] = ko.observable(data[prop] ? data[prop].get_lookupValue() : null);
                    this["edit" + prop + "Id"] = ko.observable(data[prop] ? data[prop].get_lookupId() : null);
        ...
        this.editQuestionType = ko.observable();
        this.agendaQuestionMeasures = ko.observableArray([]);
        this.editAgendaQuestionMeasures = ko.observableArray([]);

        var self = this;
        this.editQuestionType.subscribe(function (newValue) {
            if (newValue != null) {
                self.editQuestionCategoryLink(newValue.Id);
                self.editQuestionCategoryLinkId(newValue.Id);
                self.editQuestionCategoryLinkValue(newValue.Name);

                var measures = $.Enumerable
                    .From(viewModel.allMeasures)
                    .Where("$.QuestionCategoryId == " + newValue.Id)
                    .Select(function(m) { return new self.AgendaQuestionMeasure(m); })
                    .ToArray();
                self.editAgendaQuestionMeasures(measures);
            }
            else {
                self.editQuestionCategoryLink(null);
                self.editQuestionCategoryLinkId(null);
                self.editQuestionCategoryLinkValue("");
                self.editAgendaQuestionMeasures([]);
            }
        });

        this.AgendaQuestionMeasure = function (data) {
            this.MeasureId = data.Id;
            this.MeasureType = $.Enumerable.From(viewModel.allMeasureTypes).Where("$.Id == " + data.MeasureTypeId).Select("$.Name").FirstOrDefault();
            this.Unit = $.Enumerable.From(viewModel.allMeasuringUnits).Where("$.Id == " + data.UnitId).Select("$.Name").FirstOrDefault();
            this.Value = null;
        };

        this.loadQuestionMeasures = function() {
            var aqCamlQuery = new CamlBuilder().Where().LookupField("Question").Id().EqualTo(data.Id);
            var spaqQuery = new SP.CamlQuery();
            spaqQuery.set_viewXml("<View><Query>" + aqCamlQuery.ToString() + "</Query></View>");

            var questionMeasureList = $.appWebContext.get_web().get_lists().getByTitle("Показатели по вопросу");
            var questionMeasureListInst = questionMeasureList.getItems(spaqQuery);
            $.appWebContext.load(questionMeasureListInst);

            $.appWebContext.executeQueryAsync(function () {
                var enumerator = questionMeasureListInst.getEnumerator();
        ...
        this.selectLinkedQuestion = function() {
            var options = {
                ...
                url: _spPageContextInfo.webAbsoluteUrl + "/Pages/SearchQuestion.aspx",
                width: 800,
                height: 400,
                dialogReturnValueCallback: function (dialogResult, returnValue) {
        ...
        this.accept = function () {
            ...
            this.agendaQuestionMeasures(this.editAgendaQuestionMeasures());
        }.bind(this);
        ...
        this.cancel = function () {
            ...
            this.editAgendaQuestionMeasures(this.agendaQuestionMeasures());
        }.bind(this);
    ...
    function meeting(data) {
        ...
        this.changeTitle = ko.computed(function () {
            ...
            var nowDate = new Date();
            var meetingYear = this.CreatedDate == undefined ? nowDate.getFullYear() : $.datepicker.parseDate($.datepicker.regional['ru'].dateFormat, this.CreatedDate()).getFullYear();
    ...
    function loadSPEntity(fields, SPEntity) {
        ...
        for (var i = 0; i < fields.length; i++) {
            ...
            if ((fieldName.toLowerCase().indexOf("date") >= 0) && SPEntity.get_item(fieldName)) {
                entity[fieldName] = $.datepicker.formatDate($.datepicker.regional['ru'].dateFormat, SPEntity.get_item(fieldName));
            }
            else {
        ...
        entity.CreatedDate = $.datepicker.formatDate($.datepicker.regional['ru'].dateFormat, SPEntity.get_item("Created"));
        entity.New = false;
        return entity;
    }
    ...
    function saveSPEntity(fields, koItem, SPEntity) {
        for (var i = 0; i < fields.length; i++) {
            var fieldName = fields[i].get_internalName();
            if ((fieldName.toLowerCase().indexOf("date") >= 0) && koItem[fieldName]()) {
                SPEntity.set_item(fieldName, $.datepicker.parseDate($.datepicker.regional['ru'].dateFormat, koItem[fieldName]()));
            }
            else {
    ...
    function meetingViewModel() {
        ...
        self.allMeasureTypes = getMeasureTypes();
        self.allMeasuringUnits = getMeasuringUnits();
        self.allMeasures = getMeasureList();
        ...
        self.loadData = function () {
            ...
            if ($.pageMode == "new" || $.listItemId == "undefined") {
                ...
                var aqCamlQuery = new CamlBuilder().Where().LookupField("MeetingLink").Id().EqualTo($.listItemId);
                var spaqQuery = new SP.CamlQuery();
                spaqQuery.set_viewXml("<View><Query>" + aqCamlQuery.ToString() + "</Query></View>");
                ...
                $.appWebContext.executeQueryAsync(function () {
                    ...
                    while (enumerator.moveNext()) {
                        var current = enumerator.get_current();
                        var newQuest = new agendaQuestion(self, loadSPEntity(modelMetaData.agendaQuestion.fields, current));
                        ...
                        var questionType = $.Enumerable.From(self.agendaQuestionTypes()).Where("$.Id == " + newQuest.QuestionCategoryLinkId()).FirstOrDefault();
                        newQuest.editQuestionType(questionType);
                        newQuest.loadQuestionMeasures();
                    ...
                    $.appWebContext.executeQueryAsync(function () {
                        ...
                        while (aasignEnumerator.moveNext()) {
                        ...
                        while (questAttachEnum.moveNext()) {
                            ...
                            var parentQuestion = ko.utils.arrayFirst(self.agendaQuestions(), function (item) { return newAttach.MeetingLink.toString() === item.Id.toString(); });
        ...
        self.addAgendaQuestion = function () {
            ...
            self.selectedAgendaQuestion(new agendaQuestion(self, e));
        ...
        self.acceptAgendaQuestion = function () {
            ...
            if (selected.New) {
                ...
                e.New = false;
                e.MeetingLink = self.meeting().MeetingNumber();
                self.agendaQuestions.push(new agendaQuestion(self, e));
        ...
        self.save = function () {
            ...
            function () {
                ...
                saveAgendaQuestionsMeasures(self.agendaQuestions());
