...
(function () {
    ...
    function openQuestionAttachmentsDialog(title, id) {
        var options = {
            title: title,
            url: _spPageContextInfo.webAbsoluteUrl + (String).format("/_layouts/15/gradsovetpages/Pages/IssueAttachmentMVK.aspx?ID={0}", id),
            width: 800,
            height: 400,
            dialogReturnValueCallback: function (dialogResult, returnValue) {
                // ToDo: 
    ...
    function meetingViewModel() {
        ...
        //var meetingList = $.appWebContext.get_web().get_lists().getById(modelMetaData.meeting.listId);
		var meetingList = getListByUrl(modelMetaData.meeting.listName);
        //var agendaQuestionList = $.appWebContext.get_web().get_lists().getById(modelMetaData.agendaQuestion.listId);
        var agendaQuestionList = getListByUrl(modelMetaData.agendaQuestion.listName);
        //var attachList = $.appWebContext.get_web().get_lists().getById(modelMetaData.meetingAttachment.listId);
