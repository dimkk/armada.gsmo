...
(function () {
    ...
    function renderFields(context) {
        ...
        if (context.ControlMode === SPClientTemplates.ClientControlMode.DisplayForm) {
            ...
            if (a) {
                $(a).attr("id", "AssignmentLinkLink");
                assignmentLinkHtml = htmldoc.html();
            }
        }
        resultHtml += assignmentLinkHtml;

        resultHtml += renderFieldBlock('Дата', 2, 4, "AssignmentReportFactAnswerDate");
        resultHtml += '</div>';
        ...
        resultHtml += '<div class="form-group">';
        resultHtml += renderFieldBlock('Текст отчета', 2, 10, "AssignmentReportText");
        resultHtml += '</div>';

        resultHtml += '<div class="form-group" id="decision" style="display:none">';
        var resolutionHtml = renderFieldBlock('Резолюция', 2, 10, "AssignmentReportResolutionDecision");
        ...
        resultHtml += renderFieldBlockNoFormControl('Новый срок', 2, 10, "AssignmentReportResolutionNewDate");
        resultHtml += '</div>';

        resultHtml += '<div class="form-group" id="changeRequestComment" style="display:none">';
        resultHtml += renderFieldBlock('Комментарий', 2, 10, "AssignmentReportResolutionComment");
        resultHtml += '</div>';
    ...
    function getQuestionData(Id, onsuccess, onfail) {
        SP.SOD.executeOrDelayUntilScriptLoaded(function () {
            ...
            var List = ctx.get_web().get_lists().getByTitle("Вопросы повестки заседания");
            ...
            ctx.load(Instance, "Include(MeetingLink)");
    ...
    function getAssignmentData(Id, onsuccess, onfail) {
        ...
        SP.SOD.executeOrDelayUntilScriptLoaded(function () {
            ...
            ctx.load(assignmentInstance, "Include(AgendaQuestionLink, AssignmentText, AssignmentNumber)");
            ctx.executeQueryAsync(function () {
    ...
    function OnPostRender(context) {
        ...
        if (context.ControlMode === SPClientTemplates.ClientControlMode.DisplayForm) {
            ...
            renderCore.ifget('decision', function (e) {
                ...
                if (context.ListData.Items[0].AssignmentReportResolutionDecision == deadlineTriggeredValue) {
                ...
                if (context.ListData.Items[0].AssignmentReportResolutionDecision == changeRequestTriggeredValue) {
                    renderCore.ifget('changeRequestComment', function (e) {
                        $(e).css('display', 'block');
                    });
                }
            ...
            getAssignmentData(context.ListData.Items[0].AssignmentLink.split(';')[0], function (e) {
                var questionNumber = e.get_item("AgendaQuestionLink").get_lookupValue();
                var assignmentText = e.get_item("AssignmentText"); 
                var assignmentNumber = e.get_item("AssignmentNumber");;

                getQuestionData(e.get_item("AgendaQuestionLink").get_lookupId(), function (e) {
                    var meetingNumber = e.get_item("MeetingLink").get_lookupValue();
        ...
        $('#' + assignmentLinkId.replace(/(:|\.|\[|\]|\$)/g, "\\$1")).on("change", function (event) {
            if (this.value) {
                getAssignmentData(this.value, function (e) {
                    $('#LinkedAssignmentText').html(e.get_item("AssignmentText"));
                });
            }
            else {
        ...
        if (context.ControlMode === SPClientTemplates.ClientControlMode.NewForm) {
            var hasContext = document.referrer && ~document.referrer.indexOf('Lists/AssignmentList');
            if (hasContext) {
        ...
        if (context.ControlMode === SPClientTemplates.ClientControlMode.EditForm) {
            if (context.ListData.Items[0].AssignmentLink) {
                getAssignmentData(context.ListData.Items[0].AssignmentLink.split(';')[0], function (e) {
                    $('#LinkedAssignmentText').html(e.get_item("AssignmentText"));
                });
            }
    ...
    SP.SOD.executeOrDelayUntilScriptLoaded(function () {
        ...
        SP.SOD.executeOrDelayUntilScriptLoaded(function () {
            ...
            RegisterModuleInit(SPClientTemplates.Utility.ReplaceUrlTokens("~site/_layouts/15/gradsovetpages/Scripts/csr/renderAssignment.js"), init);
