...
namespace GradSovetMeetingCalendar.ControlTemplates.GradSovetMeetingCalendar
{
    public partial class LastProtocolsUserControl : UserControl
    {
        ...
        protected string ProtocolsLoad()
        {
            ...
            using (SPWeb web = SPContext.Current.Web.Site.RootWeb)
            {
                SPList meetingList = web.Lists["Заседания"];
                ...
                if (items != null)
                {
                    dataString = items.Cast<SPListItem>().Aggregate(dataString, (current, item) =>
                                                 current + string.Format("<tr align=\"center\"><td><a href=\"/_layouts/15/gradsovetpages/Pages/Meeting.aspx?ID={0}\">№ {1} от {2}</a></td></tr>",
                                                     item["ID"],
                                                     item["MeetingNumber"],
